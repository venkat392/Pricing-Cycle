...
public class HandleDAO {
	...
	private Map<Long, Double> handle_sherlock_price;
	...
	public Double getSherlockPrice(Boolean isPart, Long date) {
		Double price = Double.valueOf(0);
		if (isPart) {
			for (Map.Entry<Long, Double> entry : handle_sherlock_price.entrySet()) {
				if (entry.getKey() > date)
					return price;
				price = entry.getValue();
			}
